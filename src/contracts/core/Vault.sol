...
abstract contract VaultCore is ModuleState, Context, IVault {
     ...
     */
    // @audit not initialized tas as psm module check if needed here
    function depositLv(
        Id id,
        uint256 amount // ra ammount to deposit
    ) external override LVDepositNotPaused(id) {
        State storage state = states[id];
        // on 1st deposit it gets ratio for ds.exchangeRate
        // later deposits gets ratio from amm ctRatio
        //
