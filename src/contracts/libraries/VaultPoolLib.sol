...
library VaultPoolLibrary {
    ...
    function reserve(
        ...
        // @audit check if that can be broken
        assert(
            totalRa ==
    ...
    // @audit if there is some precission los in the sum of burnuserammoun or burnSelfammount
    // a DOS Attack could happen
    function redeem(
        VaultPool storage self,
        uint256 amount,
        address owner
    ...
    // execss is referenced as burused amount in redeem
    // attributed its burnselfammount
    {
        // loads userellegible shares from mapping prev called by user burning lv
