...
library VaultLibrary {
    ...
    // @audit check for ratio  with initial prices rounding errs
    // func called from _provideLiquiditywihRatio
    // i definetly smell smth here
    ...
    function __provideLiquidity(
        ...
        // mints ds and ct to address(this)
    ...
    // @audit - core function of protocol
    function _liquidatedLp(
        ...
        // checks for the bigger between cs ct to use that as ammount
    ...
    function redeemExpired(
    ...
    ) external returns (uint256 attributedRa, uint256 attributedPa) {
        {
            ...
            if (userEligible == 0 && !ds.isExpired()) {
                revert Unauthorized(owner); // can improve errs naming
            }
            ...
            // see how it handles if theres a ds active
            if (!ds.isExpired() && userEligible < amount) {
                revert InsufficientBalance(owner, amount, userEligible);
            }
            // @audit - q if ds isexpired and lp has not been liquidated liquidate lp
            // check for correct impl of this core func
            if (ds.isExpired() && !self.vault.lpLiquidated.get(dsId)) {
                _liquidatedLp(self, dsId, ammRouter, flashSwapRouter); // liquidates lp and gets back ra an pa
        ...
        // this func updates to 0 userElegible mapping and returns its corresponding
        //accrued PA and RA
        (attributedRa, attributedPa, burnUserAmount, burnSelfAmount) = self
            .vault
            .pool
            .redeem(amount, owner);
        // @audit - info burnselfAmount + burnUSerAmount already checked inredeem function
        // so duplicate logic here in the case of
        assert(burnSelfAmount + burnUserAmount == amount); // @audit DOS possible on any assert?
        ...
        //ra
        IERC20(self.info.pair1).safeTransfer(receiver, attributedRa); // @audit check pair0 and pair1 are not confused
        //pa
    ...
    // @audit - qwa references wrapped asset referenced as PA in the rest of the document :/
