...
library PsmLibrary {
    ...
    function onNewIssuance(
        ...
        // revert if prev pair not expired
        if (prevIdx != 0) {
    ...
    function _separateLiquidity(State storage self, uint256 prevIdx) internal {
        ...
        // @audit - q check for innecesary call
        Guard.safeAfterExpired(ds);

        uint256 availableRa = self.psm.balances.ra.convertAllToFree(); // converts all ra from locked to free, what its free?
        uint256 availablePa = self.psm.balances.paBalance;

        self.psm.poolArchive[prevIdx] = PsmPoolArchive(
            availableRa,
            availablePa,
            IERC20(ds.ct).totalSupply() // @audit - q reading from balance erc20 directly check force feed attack
        );

        // reset current balances
        self.psm.balances.ra.reset(); // sets locked and free to 0
    ...
    function deposit(
        ...
        _exchangeRate = ds.exchangeRate(); // @audit - q on empty pool how is exchangerate initialized
    ...
    // @audit - q only available with permit method??
    ...
    function _afterRedeemWithDs(
        ...
        // transfer PA ( steth... ) from sender to contract
        IERC20(self.info.peggedAsset().asErc20()).safeTransferFrom(
            owner,
            address(this),
            amount
        );
        // safe transfer RA in exchange of ds + PA
    ...
    function _afterCtRedeem(
        ...
        IERC20(self.info.peggedAsset().asErc20()).safeTransfer(
            owner,
            accruedPa
        );
